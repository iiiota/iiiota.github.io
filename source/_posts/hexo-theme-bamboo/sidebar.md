...
只需配置 `order`，越小越在前（使用flex布局的order屬性）。


## 自定义widget

创建 `source/_data/widget.yml` 文件。
...
```
若要对添加的 widget 进行UI调整，自行添加 css 到 inject。
...
- 创建 `source/githubcalendar/index.js`。
- `git_user` 改为自己的用户名。
- 自定义侧边栏的 `home_widget` 写入：
    ```yml
    home_widget:
...
创建 `source/mySwiper/index.js`、`source/mySwiper/index.css` 文件。创建 `source/mySwiper/img` 目录存放图片。
...
添加/删除轮播图，则增加/删除上面这个模块即可，这样，一个轮播图就放在了首页。
...
新建 `source/bli/index.js`、`source/bli/index.css` 文件。
...
新建 `source/weibo/index.js`、`source/weibo/index.css` 文件。
...
自定义侧边栏的`widget_library_sticky`或者`widget_library`中加入html。
```yaml
widget_library_sticky:
  - class_name: testWidget
    id_name: testWidget
...
不要热搜榜滚动的话，自己去掉`<vue-seamless-scroll></vue-seamless-scroll>`这个html标签即可。


### 历史上的今天

新建 `source/historyTody/index.js` 文件。
